...
const StatCard: React.FC<StatCardProps> = ({ title, count, subtitle, icon: Icon, delay, border }) => (
    <div
        className={`bg-black p-6 rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.08)] border ${border || 'border-gray-100'} border-l-4 hover:shadow-[0_8px_24px_rgba(0,0,0,0.1)] hover:border-gray-300 transition-all duration-300 transform hover:-translate-y-1 relative overflow-hidden group min-w-[280px]`}
        style={{ animation: `fadeInUp 0.5s ease-out ${delay}s backwards` }}
    >

        <div className="flex justify-between items-start mb-4 relative z-10">
            <div className="p-3 bg-gray-100 rounded-xl border border-gray-200 shadow-sm group-hover:bg-gray-200 transition-colors">
        ...
        <div className="relative z-10">
            <h3 className="text-white text-sm font-bold uppercase tracking-wider mb-1">{title}</h3>
            <div className="flex items-baseline gap-1">
                <span className="text-3xl font-extrabold text-white drop-shadow-sm">
                    <AnimatedCounter value={count} />
                </span>
            </div>
            <p className="text-xs text-gray-500 mt-2 font-medium bg-gray-50 inline-block px-2 py-0.5 rounded-sm border border-gray-100">{subtitle}</p>
        </div>
    </div>
);
    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Drives Calendar */}
                <div
                    className={`h-[600px] flex flex-col bg-black rounded-3xl p-4 md:p-6 shadow-md shadow-gray-200 border ${theme.border} transition-all hover:shadow-xl hover:shadow-gray-200/50 overflow-hidden`}
                    style={{ animation: 'fadeInUp 0.6s ease-out 0.4s backwards' }}
                >
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                        <div className="flex items-center gap-3">
                            <div className="w-1.5 h-6 bg-white rounded-full shadow-sm shadow-gray-300"></div>
                            <h3 className="font-bold text-white tracking-tight text-lg">Placement Schedule</h3>
                        </div>

                        <div className="relative group w-full sm:w-auto">
                            <Filter className="w-3.5 h-3.5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none group-hover:text-gray-700 transition-colors" />
                            <select
                                value={placementFilter}
                                onChange={(e) => setPlacementFilter(e.target.value as any)}
                                className="w-full sm:w-auto pl-9 pr-4 py-2 text-xs font-semibold bg-gray-50 border border-gray-200 rounded-full shadow-inner focus:ring-2 focus:ring-gray-500/20 focus:border-gray-500/50 outline-none appearance-none cursor-pointer hover:bg-white transition-all text-gray-700"
                            >
                ...
                >
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                        <div className="flex items-center gap-3">
                            <div className="w-1.5 h-6 bg-white rounded-full shadow-sm shadow-gray-300"></div>
                            <h3 className="font-bold text-white tracking-tight text-lg">Training Schedule</h3>
                        </div>
