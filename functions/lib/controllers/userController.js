...
const findUserDoc = async (uid) => {
    const promises = Object.values(ROLE_COLLECTIONS).map(async (colName) => {
        const ref = firebase_1.db.collection(colName).doc(uid);
...
router.post("/profile", async (req, res) => {
    try {
        ...
        await firebase_1.db.collection(collectionName).doc(uid).set(userProfile);
...
router.get("/role/:role", async (req, res) => {
    try {
        ...
        const snapshot = await firebase_1.db.collection(collectionName).get();
...
router.get("/all", async (req, res) => {
    try {
        ...
        for (const colName of Object.values(ROLE_COLLECTIONS)) {
            const snap = await firebase_1.db.collection(colName).get();
...
router.put("/status/roll", async (req, res) => {
    try {
        ...
        const snapshot = await firebase_1.db.collection('students').where('rollNo', '==', normalizedRoll).get();
        if (!snapshot.empty) {
